...
// Reusable Standalone Shader Background Component for login/signup
              ...
              {trustBadge.icons && (
                <div className="flex">
                  {trustBadge.icons.map((icon, idx) => (
                    <span key={idx} className={`text-${idx === 0 ? 'yellow' : idx === 1 ? 'orange' : 'amber'}-300`}>
                      {icon}
                    </span>
                  ))}
                </div>
...
// Premium Gold and Amber Theme Shader to match Qubix theme (#D4AF37)
